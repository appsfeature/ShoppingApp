...
public class AppDataManager extends DMBaseSorting {
    ...
    public void getAddressData(DynamicCallback.Listener<List<UserModel>> callback) {
        List<UserModel> savedList = ListMaintainer.getList(context, AppPreference.ADDRESS, new TypeToken<List<UserModel>>() {
        });
        if(savedList != null && savedList.size() > 0){
            callback.onSuccess(savedList);
        }
        networkManager.getAddressData(new DynamicCallback.Listener<List<UserModel>>() {
            @Override
    ...
    public void submitOrder(CartModel cartModel, AppCallback.Callback<Boolean> callback) {
        networkManager.submitOrder(cartModel, callback);
    }

    public void getOrderHistory(AppCallback.Callback<List<CartModel>> callback) {
        networkManager.getOrderHistory(callback);
    }
}
